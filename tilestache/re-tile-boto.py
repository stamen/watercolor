...
def remakeTile(url):
    ...
    if layer in cfg_dict['layers']:
        ...
        for coord in coords:
            ...
            files.append('/' + layer.name() + '/' + str(coord.zoom) + '/' + str(coord.column) + '/' + str(coord.row) + '.' + layer.preview_ext)
        return files
...
def invalidateCloudfront(files):
...
if __name__ == '__main__':
    ...
    if argv[1] == 'url':
        ...
        for url in argv[2:]:
            ...
            files = remakeTile(url)
            print files
            #invalidateCloudfront(files)
