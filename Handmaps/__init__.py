...
class Provider:
    ...
    def renderArea(self, width, height, srs, xmin, ymin, xmax, ymax, zoom):
        ...
        zoom_offset = -2
        adjusted_zoom = zoom + zoom_offset
        ...
        for layer in mapnik_layers.keys():
            ...
            mapnik_area.save("renders/" + layer + ".png")
        ...
        #
        stack = [cel for cel in self.stack if adjusted_zoom in self.parseZoom(cel["zoom"])]
        ...
        for cel_local in stack:
            ...
            if cel["type"] == "watercolor":
                comp = self.apply_watercolor_cel(comp, mask, cel, xmin, ymax, zoom, zoom_offset)
...
##        comp = comp.resize((width * 2, height * 2))
        return comp
    
    def apply_watercolor_cel(self, base, mask, cel, xmin, ymax, zoom, zoom_offset = 0):
        ...
        #
        # For the guassian radiuses, increase by a factor of 2 for each
        # zoom level offset. This is negative?
        #

        cel["edge_gauss"] *= 2**-zoom_offset
        cel["outline_gauss"] *= 2**-zoom_offset
        buff = self.layer.metatile.buffer * 2**-zoom_offset
...
##            except:
##                if trycounter >= tries:
##                    print 'giving up on watercolorization'
##                    break
##                else:
##                    trycounter += 1                
##                    pass
